function initGame() {
  ...
  const bust = (p) => `${p}?v=${Date.now()}-${Math.random().toString(36).slice(2)}`;

  // Assets
  loadSprite("bg", bust("a/kenney_platformer-kit/kenney_new-platformer-pack-1.0/Sprites/Backgrounds/Default/background_color_trees.png"));
  loadSprite("tile_block", bust("a/kenney_platformer-kit/kenney_new-platformer-pack-1.0/Sprites/Tiles/Default/terrain_stone_block.png"));
  loadSprite("player_idle", bust("a/kenney_platformer-kit/kenney_new-platformer-pack-1.0/Sprites/Characters/Default/character_green_idle.png"));
  loadSprite("goal_trophy", bust("a/kenney_platformer-kit/kenney_new-platformer-pack-1.0/Sprites/Tiles/Default/block_coin_active.png"));
  loadSprite("enemy_bee", bust("a/kenney_platformer-kit/kenney_new-platformer-pack-1.0/Sprites/Enemies/Default/bee_a.png"));
  ...
  scene("game", () => {
    ...
    player.onCollide("goal", () => {
      ...
      if (typeof showLevels === "function") showLevels();
    });
  });

  go("game");
  document.getElementById("status").textContent = `LEVEL ${currentLevel}: ${levelDef.name} - REACH THE GOLDEN TROPHY!`;
}
